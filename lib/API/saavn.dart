...
Future fetchSongDetails(songId) async {
  ...
  title = (getMain[songId]["title"]).toString().split("(")[0].replaceAll("&amp;", "&").replaceAll("&quot;", "\"");
  image = (getMain[songId]["image"]).replaceAll("150x150", "500x500");
  print((getMain[songId]["more_info"]["artistMap"]));
  artist = (getMain[songId]["more_info"]["artistMap"]["primary_artists"][0]["name"]).toString().replaceAll("&quot;", "\"").replaceAll("&amp;", "&");
  ...
  final request = new http.Request('GET', Uri.parse(kUrl))..followRedirects = false;
