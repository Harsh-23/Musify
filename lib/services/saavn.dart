...
Future<List> fetchSongsList(searchQuery) async {
  ...
  for (int i = 0; i < searchedList.length; i++) {
    searchedList[i]['title'] = fixHtmlChars(searchedList[i]['title']);
    searchedList[i]['more_info']['singers'] = fixHtmlChars(searchedList[i]['more_info']['singers']);
  }
  return searchedList;
}

Future<List> topSongs() async {
  String topSongsUrl = "https://www.jiosaavn.com/api.php?__call=webapi.get&token=8MT-LQlP35c_&type=playlist&p=1&n=20&includeMetaTags=0&ctx=web6dot0&api_version=4&_format=json&_marker=0";
  var songsListJSON = await http.get(topSongsUrl, headers: {"Accept": "application/json"});
  var songsList = json.decode(songsListJSON.body);
  topSongsList = songsList["list"];
  for (int i = 0; i < 15; i++) {
    topSongsList[i]['title'] = fixHtmlChars(topSongsList[i]['title']);
    topSongsList[i]["more_info"]["artistMap"]["primary_artists"][0]["name"] = fixHtmlChars(topSongsList[i]["more_info"]["artistMap"]["primary_artists"][0]["name"]);
    topSongsList[i]['image'] = imageQuality(topSongsList[i]['image']);
  }
  return topSongsList;
}
...
Future fetchSongDetails(songId) async {
  ...
  title = fixHtmlChars(getMain[songId]["title"]);
  image = imageQuality(getMain[songId]["image"]);
  album = fixHtmlChars(getMain[songId]["more_info"]["album"]);
  ...
  artist = fixHtmlChars(getMain[songId]["more_info"]["artistMap"]["primary_artists"][0]["name"]);
}
